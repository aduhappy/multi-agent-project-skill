...
5. 建 `文档/任务规划_<主题>.md`（从 `assets/任务规划_模板.md` 复制）。
...
7. **代码归位**：建 `scripts/` 目录 + `scripts/README.md`（从 `assets/scripts_README.md` 复制）。把用户已有的脚本列表填进去（如果有），或留空等后续 agent 填充。在 AGENTS.md §5 铁律里约定"脚本不散落根目录"。
8. 建 `STATUS.md`（从 `assets/STATUS.md` 复制）——空模板，第一个 agent 收工时填写。
...
6. **关键数字单一来源、防漂移**——关键数字（均值/百分比/面积等）只在 STATUS.md 或 AGENTS.md §3 一处写定，别处只引用不复述。收工前核对所有文档间一致性，同一数字不同文档差 1% 以上即视为 bug，必须先对齐再交。
7. **坏产物退役隔离**——发现某 agent 的产物错了（数据/图/数字），立即：①目录改名加 `_DEPRECATED` 后缀或放入 `_作废/` 子目录；②在 AGENTS.md §3 顶部用红字 `> ⚠️ 【作废】<路径>` 标注；③更新 §4 看板状态为 `[x]` + 标注"作废"；④更新 STATUS.md 反映作废。**绝不只靠记忆说"那个别用"**——下游 agent 静默复用坏产物比没产出更致命。

## 核心工作流（agent 接力全流程）

一个典型的多 agent 交接生命周期包含 5 个显式步骤。**跳过第 2 步"独立复核"是多数链路污染的根本原因**。
