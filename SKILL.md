...
### 决策登记表（必建，非可选）
把**具约束力的决策**用一张**有界、可 grep** 的表收口--一行一决策：`ID | 决策 | 取值/口径 | 理由 | 日期 | 证据链接 | 取代了谁`。放 AGENTS.md 的 3.5 节（紧邻任务看板）。它和散在 STATUS 长叙事里的自由式 ADR 不同：**有界（一决策一行，永远读得完）、可查（grep 关键词秒命中权威行）、可追（supersedes 链）**。模板在 AGENTS.md 的 6 节深读指针已预制空表。详卡存 `文档/决策记录/`。
