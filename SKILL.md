...
8. **代码归位**：建 `scripts/` + `scripts/README.md`，复制 `check_handoff.py`。`check_handoff.py` 收工自检：线级运行 `python ../scripts/check_handoff.py`（脚本在线目录里就去掉 `../`），查 §3 日期新鲜、TL;DR 已填、STATUS 非模板、STATUS 日期 ≥ §3、§4 看板、薄指针；另有 4 条只提示不阻断的提醒（决策登记表、多脚本常量漂移、文件体积、看板全未勾）；体积超限时把旧段原样移到 `文档/STATUS_归档_至<日期>.md`（AGENTS 同理），入口留一行指针。多线课题的根目录用 `--scope index` 查索引。项目里若有不认识 `--scope` 的旧副本，先整体替换成本版。
...
只有主控登记决策行；执行方、复核方只在 §3/§4 与 STATUS 记结论。
