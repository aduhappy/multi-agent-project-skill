...
6. **代码归位**：建 `scripts/` 目录 + `scripts/README.md`（从 `assets/scripts_README.md` 复制）。把用户已有的脚本列表填进去（如果有），或留空等后续 agent 填充。在 AGENTS.md §5 铁律里约定"脚本不散落根目录"。
7. 把用户的环境、约束写成 §铁律、§路径约定。
8. 跑完后告诉用户：骨架生成了哪些文件、有哪些"默认值"需要他确认、有哪些 `【待填】` 需要他补。
