...
3. 按用户勾选的工具，复制对应薄指针文件（CLAUDE.md / GEMINI.md / copilot-instructions.md 等），内容统一是"以 AGENTS.md 为准"；Cursor 用户推荐用 `.cursor/rules/multi-agent.mdc`（现代格式），`.cursorrules` 为 legacy 备用。**用户没勾的工具不要生成**。
...
7. 建 `STATUS.md`（从 `assets/STATUS.md` 复制）——空模板，第一个 agent 收工时填写。
8. 把用户的环境、约束写成 §铁律、§路径约定。
9. 跑完后告诉用户：骨架生成了哪些文件、有哪些"默认值"需要他确认、有哪些 `【待填】` 需要他补。
