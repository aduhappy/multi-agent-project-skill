...
4. **信任但验证**：agent 自检不够——关键产物按分级（见 §核心工作流的分级表）由主控或另一 agent 独立抽验通过才进下一棒；复核通过前标记"待验、下游不得启用"。
...
8. **代码归位**：建 `scripts/` + `scripts/README.md`，复制 `check_handoff.py`。`check_handoff.py` 收工自检：线级运行 `python ../scripts/check_handoff.py`（脚本在线目录里就去掉 `../`），查 §3 日期新鲜、TL;DR 已填、STATUS 非模板、STATUS 日期 ≥ §3、§4 看板、薄指针；另有 4 条只提示不阻断的提醒（决策登记表、多脚本常量漂移、文件体积、看板全未勾）；体积超限时把旧段原样移到 `文档/STATUS_归档_至<日期>.md`（AGENTS 同理），入口留一行引用（`>` 开头，不要写成带日期的标题）。多线课题的根目录用 `--scope index` 查索引。项目里若有不认识 `--scope` 的旧副本，先整体替换成本版。
...
只有主控登记决策行；执行方、复核方只在 §3/§4 与 STATUS 记结论。每格只写一两句，证据列只放链接；理由展开、复核过程写进 `文档/决策记录/`。表超过检查器 A003 体积线时，把全文原样迁到 `文档/决策记录/决策登记表_全文.md`，§3.5 改为短行索引——此后**全文是唯一权威源**，§3.5 只作索引，两处不一致以全文为准。
...
8. **谁做的要留痕**——任务卡和 STATUS 写明执行方、复核方用的工具、模型、模型家族和日期；家族按模型名写（GPT→OpenAI、Claude→Anthropic、GLM→智谱、DeepSeek→DeepSeek 等），连模型名都不知道才写 UNKNOWN；不从工具名猜。`UNKNOWN` 不算复核通过；论文结论/头条数字类复核，复核方与执行方不得同家族。
9. **关键数字换个方法核一遍**——复核方用和执行方不同的方法或来源核对；对不上记 `UNKNOWN`，不往下游放。
10. **哈希只记判据用得上的**——只给「判断会依赖它有没有变」的对象记哈希：交付物、声称没改过的输入、manifest 登记的关键产物。**不要**给每份日志、快照、报告逐个记哈希，也不要再套证据清单 + manifest + 自哈希——同机自跑自核时这是纯负担，执行方会忙着算哈希而不是做事。输入是否改过，开跑与收尾各算一次比对即可，不必把整张哈希表抄进任务卡。
...
- 🔴 **执行方有权停下来质疑卡面，且质疑本身不计为失败**：发现卡面事实有错、判据自相矛盾、或方案在当前环境不可行时，**停下来报告比按错卡面跑完一轮更有价值**。任务卡由出卡方写，**出卡方也会错——没有哪一方是不可质疑的**：卡面数字要注明出处，主控的抽查脚本同样可能有 bug。
- 🔴 **只有 `⬜未开始` 的卡可派**；还有问题没裁定的卡标 `🟠待裁定（不可派）`，不要派出去让执行方开工后才发现无从下手。
- **研究/分析类任务先写判读规则**：结果出来前写死“怎样算成立”，不因结果改动；产出先落候选文件，采纳另行裁定。

**④ 接收入库**
确认无误的产物正式落位，更新 `来源.txt`（如果是新数据）和 `scripts/README.md`（如果是新脚本）。

**⑤ handoff 落盘**
更新**线级** AGENTS.md §3 现状 + §4 看板 + 写线级 STATUS.md；根索引只写跨线增量。保证下个 agent 仅靠当前 scope 的 Markdown 能续上。🔴 主控登记推翻或暂停某个结论的决策时，同一次收工由主控在入口 TL;DR 和故事段挂红旗，不能只登记决策行——否则新 agent 会照着已作废的结论写。
