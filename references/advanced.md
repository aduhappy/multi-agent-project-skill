...
决策记录解决一件事：**防止下个 agent 重新踩坑、重新质疑已定的事，或抄到一个早被推翻的口径。** 但自由式 ADR 散在文件夹里、或埋在 STATUS 长叙事里，**不可查、不可数**——长日志没人读全（接力事故的高发区）。所以分两层：

### 1a. 决策登记表（有界、可 grep —— 强烈推荐，优先于详卡）

一张表收口所有**具约束力**的决策，**一行一决策**，放 AGENTS.md 紧邻 §3 处或单独 `文档/决策登记表.md`（§6 放指针）：

```markdown
## 决策登记表（具约束力的决策，单一权威源）
| ID | 状态 | 决策 | 取值/口径 | 理由 | 日期 | 证据 | 取代 | 被谁取代 |
|----|------|------|-----------|------|------|------|------|----------|
| D-03 | ACTIVE | fPOC 拟合排除 H28 | H28 非年龄序列（3 树种横向比较） | 物种效应≠年龄效应 | 2026-06-25 | →文档/H28复核.md | 取代 e5g 留 H28 口径 | — |
| D-04 | ACTIVE | 退耕成熟值只报区间 | 0.45–0.55，不给单值 | >40yr 零实测 | 2026-06-27 | →E8 决策卡 | — | — |
```

为什么是这张表、不是文件夹散卡：
- **有界**——一决策一行，永远读得完，符合"必读层"要求（铁律 5）。
- **可查**——`grep H28 文档/决策登记表.md` 秒命中权威行，不用扫 600 行 STATUS、也不用猜哪个脚本对。
- **可追**——`取代` 列把"新决策废了哪条旧口径"显式连起来；`被谁取代` 列反向链接，旧决策被取代时指向新 ID。双向链确保任何 agent 读到旧行都能顺着找到当前有效版本，防止旧口径借滞后脚本还魂。变更时旧行标 `SUPERSEDED`，不能静默覆盖。

> **判据**：任何"用哪些点 / 剔哪些 / 选哪个参数 / 走哪条路线"的决策，都该在这张表里有一行。建模/取数/复现前**先 grep 这张表**，别先信某个脚本的硬编码。

### 1b. 决策详卡 ADR（登记表某行需要展开时才写）

`文档/决策记录/<日期>_<主题>.md`，登记表里复杂的那几条再展开：
```markdown
# 决策：<一句话结论>
- 日期：YYYY-MM-DD
- 状态：采纳 / 已废弃 / 待定
...
### 1c. 口径参数 = 代码的单一来源（防多脚本编码漂移）

登记表治"决策写在哪"，这条治"代码怎么不跑偏"。**口径参数 / 样本集 / 排除清单不准在多个脚本里各自硬编码**——抽进唯一 config，所有脚本读它：

```python
# config/参数.py（或 .yaml）——唯一权威，对应决策登记表
AFF_STUDIES = {"4", "36", "H27"}          # 纳入拟合（D-02）
DROP_STUDIES = {"C6", "H28", "H15"}        # 排除（D-03，附理由见登记表）
```
```python
# 任何脚本里
from config.参数 import AFF_STUDIES, DROP_STUDIES   # 读，不复述
```

> **接力事故原型**：脚本 A 写 `AFF={"4","36","H27","H28"}`、脚本 B 写 `{"4","36","H27"}`——同一集合两份相反编码。下一个 agent 抄了带 H28 的那份，坏点把 R² 从 0.5 拖到 0.27，汇报被推翻。自检看不出、格式检查也看不出。**单一 config + 决策登记表**两层一起，才堵得住。

### 1d. 被取代的脚本要退役（不只数据产物）

某决策一旦改了方法，**被它取代的旧脚本同样退役**：脚本顶部加注释，别让它当活口径把下家钓进去。
```python
# _DEPRECATED 2026-06-27 → 口径已改，见决策登记表 D-03 / 用 e5k3_*.py
```
