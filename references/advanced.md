...
## 5. 标准 handoff 摘要 + STATUS.md（交接落盘文件）

agent 收工时（除更新 AGENTS.md 外）写/更新 **`STATUS.md`**（放在项目根目录）。这是下个 agent 的第一眼——**读这个就能一秒接上当前状态，不依赖对话历史**（对话可能被压缩/重置，文件不会）。
...
- 下一步：<第一个动作是什么，精确到命令/文件>
- 未解决风险：<需要下个 agent 注意的坑>
- 关键文件路径：<3–5 个最重要的入口文件>
- 阻塞：<需用户确认 Z / 等数据下载 / ...>
```

在 AGENTS.md §3 "现在在哪"里放一行指针："`→ 当前详细 handoff 见 STATUS.md`"，§3 只保留最新 1-2 条，细节下沉。
