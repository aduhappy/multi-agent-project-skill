...
|----|------|-----------|------|------|------|------|
| D-03 | fPOC 拟合排除 H28 | H28 非年龄序列（3 树种横向比较） | 物种效应≠年龄效应 | 2026-06-25 | →文档/H28复核.md | 取代 e5g 留 H28 口径 |
| D-04 | 退耕成熟值只报区间 | 0.45–0.55，不给单值 | >40yr 零实测 | 2026-06-27 | →E8 决策卡 | — |
```

为什么是这张表、不是文件夹散卡：
- **有界**——一决策一行，永远读得完，符合"必读层"要求（铁律 5）。
- **可查**——`grep H28 文档/决策登记表.md` 秒命中权威行，不用扫 600 行 STATUS、也不用猜哪个脚本对。
- **可追**——`取代` 列把"新决策废了哪条旧口径"显式连起来，防止旧口径借滞后脚本还魂。
