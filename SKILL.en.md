...
> **Decision registry (strongly recommended — don't treat it as advanced-only)**: collect every *binding* decision into one **bounded, greppable** table — one row each: `ID | Decision | Value/method | Reason | Date | Evidence | Supersedes`. Place it under the AGENTS.md §6 pointer or next to §3. Unlike free-form ADRs scattered in a folder or buried in the STATUS narrative, this table is **bounded** (one line per decision, always fully readable), **queryable** (grep a keyword, hit the authoritative row), and **traceable** (the supersedes column links a new decision to the old convention it kills). It's the key layer that prevents a decision from being buried in a long log or silently overridden by a stale script. Template in `references/advanced.md` §1.
...
5. **New agent (including sub-agents) first thing on entry** — Read AGENTS.md (including STATUS.md) before doing anything. Don't rely on conversation history or memory guesses. **STATUS.md grows until nobody reads it in full** — so any *binding* decision (methodology / exclusion list / chosen parameters) must be lifted into a **bounded, must-read** layer: AGENTS.md §3 or a **decision registry**. Don't leave it only in the long STATUS narrative. Before pulling data / modeling / reproducing, query that layer first — don't trust a constant hardcoded in some script.
6. **Single source for key numbers AND methodology parameters, prevent drift** — Key numbers (means, percentages, areas) written in exactly one place (STATUS.md or AGENTS.md §3); everywhere else references but doesn't restate. **The same applies to methodology parameters / sample sets / exclusion lists (which points to use, which to drop, what thresholds): pull them into ONE config (`config/params.yaml` or one authoritative table) that every script reads — never hardcode the same set in multiple scripts.** Two scripts encoding the same set differently is the most insidious handoff failure (self-checks miss it, format checks miss it). Before shutdown, verify consistency across all docs **and scripts** — a >1% numeric difference, or a membership mismatch for the same set, is a bug that must be fixed before handoff.
7. **Retire bad outputs AND superseded scripts** — When an agent's output is found wrong (data, figures, numbers), immediately: ① rename directory/file with `_DEPRECATED` suffix or move to `_retired/`; ② annotate at top of AGENTS.md §3 with `> ⚠️ 【RETIRED】<path> <reason>`; ③ update §4 task board status to `[x]` + "retired"; ④ update STATUS.md; ⑤ **a script/method superseded by a decision is retired too** — add a `# _DEPRECATED → see <authoritative script/decision>` header so it can't pose as a live convention and hook the next agent. **Never rely on memory saying "don't use that"** — a downstream agent silently reusing a bad output, or copying a stale script's convention, is more damaging than no output.

> 💡 **A real handoff failure (this is where rules 5/6/7 come from)**: an agent re-fitting a curve copied a sample set from a **stale script** — that script still kept a bad point that should have been dropped, while the "drop this point" decision sat buried deep in a 600-line STATUS, and two scripts encoded it in opposite ways. The bad point dragged R² from 0.5 down to 0.27 and the report was overturned. In hindsight all three layers could have caught it: the decision wasn't lifted into a bounded must-read layer (rule 5), the sample set was hardcoded across scripts instead of one config (rule 6), and the superseded script wasn't marked retired (rule 7). **Trusting a stale script over the decision record** is the common root cause.
